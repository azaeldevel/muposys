...
BodyUserPermission::BodyUserPermission()
{
	...
	userlst = muposysdb::User::select(conn,"",0);
	for(auto u : *userlst)
	{
		u->downName(conn);
	...
	permisslst = muposysdb::Permission::select(conn,"",50);
...
bool UserPermission::methode(std::ostream& out)
{
	...
	/*try
	{
		...
		userlst = muposysdb::User::select(connDB,strsql,0,'D');
		if(not userlst->size()) return false;
		if(userlst->size() != 1) return false;
		muposysdb::User* user = userlst->front();
