...
CREATE TABLE Catalog_Item(id INT  PRIMARY KEY NOT NULL,catalog INT NOT NULL,number VARCHAR(60) NOT NULL,name VARCHAR(60) NOT NULL,FOREIGN KEY(catalog) REFERENCES Catalog(catalog));
ALTER TABLE Catalog_Item ADD CONSTRAINT Cataloging_number_unique UNIQUE (catalog,number);
ALTER TABLE Catalog_Item ADD COLUMN active ENUM('Y','N');
ALTER TABLE Catalog_Item ADD CONSTRAINT Cataloging_Entity_id FOREIGN KEY(id) REFERENCES Ente(id) ;
ALTER TABLE Catalog_Item ADD COLUMN value FLOAT;
ALTER TABLE Catalog_Item ADD COLUMN presentation VARCHAR(5);
...
-- 
CREATE TABLE Stocking (id INT  PRIMARY KEY NOT NULL,stock INT NOT NULL,item INT NOT NULL,amount INT NOT NULL,FOREIGN KEY(stock) REFERENCES Stock(stock),FOREIGN KEY(item) REFERENCES Catalog_Item(id));
